...
const request = (path, fallback) => fetch(`${path}?_t=${Date.now()}`).then((response) => {
...
function chapterCard(chapter) {
  const quick = chapter.reading_priority === "quick_read" && chapter.retain_if_quick_read?.length
    ? `<div class="quick-retain" style="background:#fef3c7;padding:8px 12px;border-radius:6px;margin:8px 0;border-left:4px solid #f59e0b;"><strong style="color:#b45309;">快读保留要点：</strong><ul style="margin:4px 0 0 18px;padding:0;">${chapter.retain_if_quick_read.map((f)=>`<li>${escapeHtml(f)}</li>`).join("")}</ul></div>` : "";
  ...
  const events = chapter.key_events?.length ? `<div class="key-events" style="margin:8px 0;"><strong>核心事件：</strong><ul style="margin:4px 0 0 18px;padding:0;">${chapter.key_events.map((e)=>`<li>${escapeHtml(e)}</li>`).join("")}</ul></div>` : "";
  ...
  return `<article class="card ${escapeHtml(chapter.reading_priority || "quick_read")}" id="chapter-${chapter.id}">
    ...
    <p><strong>阅读建议：</strong>${escapeHtml(chapter.priority_reason || "未提供")}</p>
    <p><strong>涉及人物：</strong>${escapeHtml(people || "未标注")}</p>
    <p class="teaser-text" style="color:#4b5563;">${escapeHtml(chapter.teaser || "")}</p>
    ${tags(chapter.content_tags, labelContentTag)}${tags(chapter.narrative_roles, labelNarrativeRole)}
    <details open style="margin-top:10px;"><summary style="cursor:pointer;font-weight:bold;color:#2563eb;">📖 剧情摘要（含剧透）</summary>
      <div style="background:#f9fafb;padding:12px;border-radius:6px;margin:8px 0;line-height:1.6;white-space:pre-wrap;" class="chapter-summary">${escapeHtml(chapter.summary || "暂无摘要")}</div>
      ${events}${changes}${quick}${evidence}
    </details>
    ${chapter.text_asset ? `<p style="margin-top:8px;"><button class="load-text" data-text="${escapeHtml(chapter.text_asset)}" data-loaded="false">阅读本章全文</button></p><div class="text-slot"></div>` : ""}
  </article>`;
}
